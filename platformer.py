...
def draw_button(text, x, y, w, h):
    draw_text(text, font_score, blue,x + 20, y + 15)
    return pygame.draw.rect(screen, BUTTON_COLOR, (x, y, w, h), border_radius=12)
...
while run:
	...
	if main_menu == True:
		...
		#if player has died
		if game_over == -1 and show_continue_prompt:
			draw_text('GAME OVER!\nDO YOU WANT TO CONTINUE?', font, blue, screen_width // 2, screen_height // 2 - 100)

			if watch_button.draw():
				show_continue_prompt = False

			if answer_button.draw():
				show_continue_prompt = False
				question_mode = True
